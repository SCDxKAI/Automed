...
class Asilify {
     ...
     * @return mixed 
     */
    public static function limits($user, $type) {

        $response = ( object ) array("allow" => false, "message" => "");

        if ($user->parent->admin == "Yes") {
            $response->allow = true;
            return $response;
        }

        $plan = Database::table("plans")->where("id", $user->parent->subscription_plan)->first();
        if ($type == "users") {
            $users = Database::table("users")->where("company", $user->company)->count("id", "total")[0]->total;
            $users = 2;
            if (empty($plan->users_limit) || $users < $plan->users_limit) {
                $response->allow = true;
                return $response;
            }

            $response->message = "You have reached your limit of ".$plan->users_limit." users. You can upgrade you plan to add more users.";
        }elseif ($type == "projects") {
            $projects = Database::table("projects")->where("company", $user->company)->count("id", "total")[0]->total;
            if (empty($plan->projects_limit) || $projects < $plan->projects_limit) {
                $response->allow = true;
                return $response;
            }

            $response->message = "You have reached your limit of ".$plan->projects_limit." projects. You can upgrade you plan to add more projects.";
        }
