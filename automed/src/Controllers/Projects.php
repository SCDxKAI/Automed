...
class Projects {
    ...
    public function create() {
        ...
        $user = Auth::user();
        $limit = Asilify::limits($user, "projects");
        if (!$limit->allow) {
            return response()->json(responder("warning", "Hmmm!", $limit->message));
        }
    ...
    public function details($projectid) {
        ...
        return view("project-details", compact("user", "title", "client","notes","project","staffmembers","tasks","expenses","quotes","invoices","payments","jobcards","suppliers","inventory"));
    ...
    public function booking() {
        ...
        if($user->parent->booking_form == "Detailed"){
            $partslist = Database::table('partslist')->where('company', $user->company)->orderBy("indexing", true)->get();
            return view('modals/project-booking-detailed', compact("insurance","clients","user","partslist","makes"));
        }else{
            return view('modals/project-booking-basic', compact("insurance","clients","user","makes"));
        }
