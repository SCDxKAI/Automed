...
class Settings {
    ...
    public function updatesystem() {
        // return response()->json(responder("warning", "Demo Mode", "Your request was received but not processed since this is a demo environment."));

        $envPath = str_replace("src/Controllers", ".env", dirname(__FILE__));
        $env = new DotEnvWriter($envPath);
        $env->castBooleans();

        $options = array("APP_NAME", "SMS_PROVIDER","AFRICASTALKING_USERNAME","AFRICASTALKING_KEY","AFRICASTALKING_SENDERID","TWILIO_SID","TWILIO_AUTHTOKEN","TWILIO_PHONENUMBER","MAIL_USERNAME","MAIL_SENDER","SMTP_HOST","SMTP_PASSWORD","SMTP_PORT","MAIL_ENCRYPTION","STRIPE_PUBLISHABLE_KEY","STRIPE_SECRET_KEY");
