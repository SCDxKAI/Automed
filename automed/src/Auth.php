...
class Auth {
    ...
    public static function login($username, $password, $options = array()) {
        ...
        $user = Database::table(config('auth.table'))->where("email",$username)->where("role","Admin")->orWhere("email",$username)->where("role","Owner")->orWhere("email",$username)->where("role","Manager")->orWhere("email",$username)->where("role","Inventory Manager")->orWhere("email",$username)->where("role","Booking Manager")->first();
